...
float stepcount = 0.3;
int stepdir = 1;
float sep = random(1.5,8.5);
float coh = random (1.5,8.5);
float ali = random(1.5,8.5);
int sepdir = 1;
int cohdir = 1;
int alidir = 1;
int countdir = 1;
void draw(){
  ...
  EVERY_N_MILLISECONDS(8000) {  //every 8 seconds it will change the variables
   ...
   boidseperation = random(4,10);
   neighbordistance = random(4,10);
   fadebyvalue = random(1,8)*20;
   if (sep < 1.5 || sep > 8.5) sepdir = -sepdir;
    if (coh < 1.5 || coh > 8.5) cohdir = -cohdir;
    if (ali < 1.5 || ali > 8.5) alidir = -alidir;
  ...
  for (int i = 0; i < count; i++) {
    ...
    drawPixelXYF(boid->location.x, boid->location.y, ColorFromPalette(RedThroughBlueColors_p,boid->hue*15,255,NOBLEND));            
